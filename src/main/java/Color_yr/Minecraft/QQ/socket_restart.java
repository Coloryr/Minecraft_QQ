...
public class socket_restart {
	...
	public boolean socket_restart_start() {
		Minecraft_QQ_bungee.log.info("§d[Minecraft_QQ_bungee]§5正在连接酷Q");
		if (logs.Socket_log == true) {
			logs logs = new logs();
			logs.log_write("[socket]正在连接酷Q");
		}
		try {
			socket.socket = new Socket(Minecraft_QQ_bungee.System_IP, Minecraft_QQ_bungee.System_PORT);

			socket.socket_runFlag = true;
			Minecraft_QQ_bungee.log.info("§d[Minecraft_QQ_bungee]§5酷Q已连接");
			if (logs.Socket_log == true) {
				logs logs = new logs();
			...
			Minecraft_QQ_bungee.log.warning("§d[Minecraft_QQ_bungee]§c酷Q连接失败");
			if (logs.Socket_log == true) {
				logs logs = new logs();
