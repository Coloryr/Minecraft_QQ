...
public class BungeeCord implements Listener {
    ...
    public void onPostLogin(PostLoginEvent event) {
        if (socket.hand.socket_runFlag && Bukkit.Join_sendQQ) {
    ...
    public void onPlayerquit(PlayerDisconnectEvent event) {
        if (socket.hand.socket_runFlag && Bukkit.Quit_sendQQ) {
            String playerName = event.getPlayer().getName();
            socket_send.send_data(Placeholder.data, Placeholder.group,
    ...
    public void onChar(ChatEvent event) {
        ...
        if (Bukkit.User_NotSendCommder) {
            if (player_message.indexOf("/") == 0)
                return;
        } else if (Bukkit.Mute_List.contains(player.getName()))
            return;
        if (Bukkit.Minecraft_Mode != 0 && socket.hand.socket_runFlag) {
            ...
            if (Bukkit.User_SendSucceed && send_ok)
