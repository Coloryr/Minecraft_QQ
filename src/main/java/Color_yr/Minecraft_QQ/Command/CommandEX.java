...
public class CommandEX {
    ...
    private static void reload(Object sender) {
        Minecraft_QQ.Side.send(sender, "§d[Minecraft_QQ]§e机器人IP： " + Minecraft_QQ.Config.getSystem().getIP());
        Minecraft_QQ.Side.send(sender, "§d[Minecraft_QQ]§e机器人端口 " + Minecraft_QQ.Config.getSystem().getPort());
        Minecraft_QQ.Side.send(sender, "§d[Minecraft_QQ]§e机器人模式 " + Minecraft_QQ.Config.getServerSet().getMode());
        Minecraft_QQ.Side.send(sender, "§d[Minecraft_QQ]§eDebug模式 " + Minecraft_QQ.Config.getSystem().isDebug());
    ...
    public static void Ex(Object sender, String name, String[] args, boolean hasPermission) {
        ...
        if (args[0].equalsIgnoreCase("reload") && hasPermission) {
            ...
            if (!Minecraft_QQ.Config.getMute().contains(name)) {
                ...
                Minecraft_QQ.Side.send(sender, "§d[Minecraft_QQ]" + Minecraft_QQ.Config.getLanguage().getMessageOFF());
            } else {
                Minecraft_QQ.Config.RemoveMute(name);
                Minecraft_QQ.Side.send(sender, "§d[Minecraft_QQ]" + Minecraft_QQ.Config.getLanguage().getMessageON());
            }
            Minecraft_QQ.save();
        } else {
            Minecraft_QQ.Side.send(sender, "§d[Minecraft_QQ]§c错误，请使用/qq help 获取帮助");
